...
- 출처 묶음(2026-09-25, REQUIREMENTS F-004): 사용자설정·관리자설정의 공공 출처가 "입찰 공고" / "지원사업 (선택) · 기본 꺼짐" 두 묶음이고
  보조금24가 보인다. 지원사업 출처를 켜면 입찰공고 목록에서 그 공고 출처 옆에 보라색 "지원" 배지.
  추가: 메뉴 "입찰(지원사업) 공고", 출처 선택 박스의 "전체 출처(입찰)"·"전체 출처(지원)"가 그 묶음만 보여준다(건수도).
- 알리오 상세 보강(2026-09-25, bid-collectors v1.3.0): 알리오 공고를 열면 곧바로 뜨고 1~2초 뒤 첨부파일 목록과 "원문 바로가기 (나라장터/온비드…)"
  주 버튼 + "알리오 공고" 보조 버튼이 생긴다. 나라장터에도 있는 공고(DB에 수집된 것)는 "연결된 공고 → 나라장터 공고 보기". 같은 공고를
  다시 열면 로딩 표시가 짧다(재조회 안 함). 실측: ALIO-3580304 첨부 6개·원문 g2b — 그 나라장터 번호는 DB 수집 범위(9/18~24) 밖이라 연결 없음이 정상.
