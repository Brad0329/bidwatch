...
- **관리자설정 — 기본 제공 사이트** (2026-09-24, 스키마 004 — 일반 트랙: 이 확인 전 배포 금지): 목록에 39곳(사용 가능 35 ·
  수집 불가 4)이 이름·상태·최근 수집·주소로 나오는지 / 주소 하나를 사용자설정 > 직접 추가한 사이트에 넣으면 **분석 없이 즉시
...
- **기본 제공 사이트 실패 4곳** (2026-09-24 `import_builtin_sites.py`): 신용보증기금(alio.go.kr — `{{ item.rtitle }}` JS 렌더링,
  AI 3회 실패, GET 스크래핑으로는 구조적 불가) / 한국산업기술진흥원(500) / 충남테크노파크(503) / 한국디자인진흥원(타임아웃).
  뒤 셋은 사이트 장애 — 살아나면 `import_builtin_sites.py --retry-failed`(AI 과금). 정기 수집(Phase 009)이 생기면 기본 제공 35곳도
  그 대상이다 — 지금은 등록 시 첫 수집(최근 30일) 1회뿐.
