...
- **사용자설정 > 구독 출처 > 직접 추가한 사이트** (2026-09-23): URL 입력 → 목록에 "AI 분석 중" → 1분 안에
  ...
  "사용 가능"이 되면 **공고 목록**에 그 사이트 공고가 나오는지(키워드와 맞는 것만) / 열어서 상세·태그가 되는지 /
  구독 해지 후 목록에서 사라지는지.
- **직접 추가한 사이트 — 등록 이름 확인 카드** (2026-09-24): "사용 가능"이 되면 항목 아래 카드가 펼쳐지고 AI가 읽은 이름이
  기본값인지 / 이름을 고쳐 [확인] → 카드가 닫히고 목록·공고 목록 출처에 고친 이름 / [취소] → 구독 해지 /
  이름을 모두 지우면 [확인]이 눌리지 않는지(고장 시나리오). 기존에 이름 없이 "사용 가능"이던 사이트에도 카드가 뜬다.
- **공고 목록 — 출처 선택 박스** (2026-09-24): 검색창 왼쪽 "전체 출처" → 공공 출처/직접 추가한 사이트 묶음으로 나오고,
  사이트는 위에서 정한 이름으로 보이는지 / 고르면 그 출처 공고만·총 건수도 그 기준인지 / 검색어·태그·지역과 함께 걸리는지.
  입찰 예고에도 같은 박스("전체 출처"/나라장터 사전규격 — 1개 고정이라 목록은 같다).
