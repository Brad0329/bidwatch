...
- **지역 필터 — 관심 지역 자동 적용 폐기** (2026-09-24): 공고 목록·입찰 예고·검토요청을 열면 지역 필터가 비어 있는지(전체) /
  사용자설정에 "관심 지역" 칸이 없는지 / 지역을 고르면 그 지역 공고와 함께 알리오·사전규격·직접 추가 사이트 공고가 남는지 /
  공고를 열면 지역이 "지역 미상"으로 보이는지.
- **관리자설정 — 알리오 공공 출처** (2026-09-24, 마이그레이션 005 — 일반 트랙: 이 확인 전 배포 금지): 수집 관리에
  "알리오 공공기관 입찰공고"가 나오는지 / 체크(구독) → [수집] → 공고 목록에 수자원공사·코레일 등 기관명과 함께 나오는지
  (개발 DB에는 이미 474건 수집됨, 9/22~24) / 공고 목록 출처 선택 박스에 알리오가 보이는지.
- **관리자설정 — 기본 제공 사이트** (2026-09-24, 스키마 004 — 일반 트랙: 이 확인 전 배포 금지): 목록에 38곳(사용 가능 35 ·
  수집 불가 3)이 이름·상태·최근 수집·주소로 나오는지 / 주소 하나를 사용자설정 > 직접 추가한 사이트에 넣으면 **분석 없이 즉시
  "사용 가능"** + 이름 확인 카드가 뜨는지 / 그 뒤 공고 목록에 그 기관 공고가 나오는지 / member 계정에는 관리자설정이 안 보이는지.
