...
## 사용자 실테스트 대기
- (없음) — 2026-09-24 사용자가 전부 확인: 직접 추가한 사이트(2026-09-23)·등록 이름 확인 카드·출처 선택 박스·
...
- **알리오 수집 누락 — bid-collectors 결함 (2026-09-24 발견, 미수정)**: 목록이 등록 순이라 공고일이 뒤섞이는데 AlioCollector가
  처음 나온 오래된 항목에서 멈춰 9/22 공고 472건을 조용히 놓쳤다. 하루 공고량도 약 450~500건(max_pages 50 ≈ 하루치).
  bid-collectors 세션에서 수정(한 페이지 전체가 기준일 이전일 때 멈춤·max_pages 상향·뒤섞인 목록 테스트) → bidwatch에서 재수집해
  9/22 약 500건 확인. 상세 `procurement_sources_research.md` 3-3.
