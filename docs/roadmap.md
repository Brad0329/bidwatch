...
Phase 1-7: 태그 시스템                                                ✅
  ├── 태그 API (PUT upsert, DELETE, GET 목록/단건)
  ├── 공고 목록에서 태그 배지 표시
  ├── 공고 모달에서 태그 선택/변경/삭제
  └── 태그 필터 버튼 (공고 목록 + 입찰 예고)

Phase 1-8: 지역 필터링                                                ✅
  ├── 17개 시/도 정규화 (수집 시점 적용)
  ├── 공고 목록/입찰 예고 API에 region 필터 파라미터
  ├── 지역 목록 API (/api/notices/regions)
  ├── 프로필 관심 지역 API (/api/profile/regions)
  ├── 멀티셀렉트 지역 드롭다운 (공고 목록, 입찰 예고, 검토요청)
  └── 사용자 설정 관심 지역 섹션 + 기본 필터 자동 적용
```
